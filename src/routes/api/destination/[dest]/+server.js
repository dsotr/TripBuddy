...
export async function GET({ params }) {
	...
	const { data, error } = await supabase.from('destinations').select().eq('name', dest);

	if (error || data.length == 0) {
		const piaxabayImage = fetch(
			`https://pixabay.com/api/?key=${PIXABAY_KEY}&q=${dest}&image_type=photo&safesearch=true`
		);
	}
	...
	const pixabayUrl = `https://pixabay.com/api/?key=${PIXABAY_KEY}&q=${encodeURIComponent(dest)}&image_type=photo&safesearch=true`;
	...
	const imageBuffer = await (await imageBytes).arrayBuffer();
